...
public class Ambulance implements CProcess, ProductAcceptor {
	...
	public void execute(int type, double tme) {
		...
		StatsCollector.collectEvent(eventlist.getTime(), type, patient.getPatientID(), name,"Patient dropped off at the hospital","");		
		patient = null;
	...
	public boolean givePatient(Patient p) {
		...
		if (status == 'i') {
			...
			patient = p;
			// mark starting time
			//printStartedProduction(double time, Ambulance ambulance, Patient patient)
			...
			//*****************
			// BEGIN addition Anna 
			//method signature: collectEvent(double elapsedTime, String stateType, String patientID, String dockName, UUID machineID ) 
			//System.out.println("in ambulance 2, machien parameter: "+name);
			//StatsCollector.collectEvent(eventlist.getTime(), 888, patient.getPatientID(), name,"Patient picked up","");
			// END addition Anna 
			//*****************
			// start production
			handlePatient();
