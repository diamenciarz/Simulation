	...
	public boolean givePatient(Patient patient) {
		...
		if (idleMachines.size() < 1)
			...
			while (!delivered & (idleMachines.size() > 0)) {
				...
				System.out.println("in queue, machien parameter: "+closestAmbulance.name);
				StatsCollector.collectEvent(111, 888, patient.getPatientID(), closestAmbulance.name,queue.size()+" elements in the queue in givePatient in Queue","");
